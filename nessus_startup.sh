...
ln -sf /dev/stdout /opt/nessus/var/nessus/logs/nessusd.messages 2>/dev/null
ln -sf /dev/stdout /opt/nessus/var/nessus/logs/www_server.log 2>/dev/null
ln -sf /dev/stdout /opt/nessus/var/nessus/logs/backend.log 2>/dev/null

if [ ! -n "${NON_PRIV_USER}" ]; then
    NON_PRIV_USER=nessus
fi

if [ "${PRODUCT_INITIALIZATION}" = "true" ]; then

    rm -rf /opt/nessus/* 2>/dev/null
    cp -r /first_start/nessus/ /opt/
    ...
    echo "-- Skip New Nessus Installation, Attempting to start Existing Installation"
fi
